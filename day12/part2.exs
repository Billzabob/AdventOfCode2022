defmodule Day12 do
  def run() do
    ...
    starts = map |> Enum.filter(fn {_k, v} -> v == ?a end) |> Enum.map(fn {k, _v} -> k end)
    {finish, _} = Enum.find(map, fn {_k, v} -> v == ?E end)

    map = map |> Map.put(start, ?a) |> Map.put(finish, ?z)

    [start | starts]
    |> Enum.map(fn start -> bfs(map, [start], MapSet.new(), 1, finish) end)
    |> Enum.min()
    |> IO.puts()
  end
